...
def load_and_plot(dir):
    ...
    mean = dict["mean"]
    n = 25
    texify_most_informative_features_less(sort_p_lower,sort_p_upper,mean,n)
    ...
    for i in range(len(plot_index)):
        ...
        plt.title(str(sort_p_lower[plot_index[i]][1]).replace("95% ci","95% CI"))
        if(i==2):
    ...
    plt.savefig(dir+"/top_fea_pos" + "_L2.pdf")
    ...
    for i in range(len(plot_index)):
        ...
        plt.tick_params(axis='y',left='off',labelleft='off')
    #save figure as pdf file
    plt.savefig(dir+"/top_fea_neg" + "_L2.pdf")
...
def texify_most_informative_features(sort_p_lower,sort_p_upper,mean,n):
    ...
    while i<n:
        ...
        out_str.append("%.5f$\pm$%.5f & %s & %.5f$\pm$%.5f & %s \\\\" % (mean1, diff1,name1,mean2,diff2,name2))
        i += 1
...
def texify_most_informative_features_less(sort_p_lower,sort_p_upper,mean,n):
    out_str = [
        r'''\begin{table}
            \caption{top 25 features for press release positive prediction}
            \begin{tabular}{l c|l c}
    ...
    while i<n:
        ...
        #diff2 = mean2 - bound2
        name2 = sort_p_lower[i][1]
        out_str.append("%s & %s \\\\" % (name1,name2))
        i += 1
