...
def load_articles(articles_dir="1. excel files"):
    ...
    for article_file_name in article_files:
        ...
        #print article ID with high ranked features
        if 'UK' in abstract and 'England' in abstract:
            print str(article["ID"])+' has UK and England'
        if 'death' in abstract and 'infection' in abstract:
            print str(article["ID"])+' has death and infection and alcohol'
        #calcuate the total legnth of title and abstracts
...
def load_matched_samples(matched_dir="7. Matched samples"):
    ...
    for article_file_name in article_files:
        ...
        abstract = article["abstract"]
        title = article["title"]
        if 'mice' in abstract and 'signaling' in abstract and 'research' in abstract:
            print str(article["ID"]) + "has mice"
        #len_abstract += len(TextBlob(abstract).words)
        #len_title += len(TextBlob(title).words)
...
def read_in_article(article_path):
    ...
    return {"ID":input_file.name,"pmid":pmid, "title":title, "mesh":mesh, "authors":authors,
